...
Dabra is designed to be **_modular_**, allowing for the addition of **_plugins_**
...
Example plugins are:
- **[Quadratic Voting](/quadratic.mdx)**: The voter weight of a member is proportional to the square root of the number of tokens they hold.
- **[Voter Stake Registry](/vsr.mdx)**: A voter's weight can be increased by locking tokens.
- **[NFT](/setup/daonft.mdx)**: The voter weight of a member is proportional to the number and type of NFTs they hold.
...
Plugins are stored in the [Goverance Program Library](https://github.com/dabra-foundation/governance-program-library), where you can see example plugins.
...
This associates a plugin with a dabra, and stores any configuration data that this plugin uses.
...
but also the total maximum votes of a dabra. An example is an NFT DAO where the total maximum votes
...
pub struct Registrar {
    /// spl-governance program the Dabra belongs to
    pub governance_program_id: Pubkey,

    /// Dabra of the Registrar
    pub dabra: Pubkey,

    /// Governing token mint the Registrar is for
    /// It can either be the Community or the Council mint of the Dabra
...
See [here](https://github.com/dabra-foundation/solana-program-library/blob/master/governance/addin-api/src/voter_weight.rs) for its structure.
...
The JS/TS client is a JS plugin for the Dabra UI, allowing the UI to interact with your plugin program
at voting time, and when a user joins a DAO.

The client must implement the [Client interface](https://github.com/dabra-foundation/governance-program-library/blob/master/src/common/Client.ts) in governance-program-library.
...
## Integrating with Dabra

Once you have a program and client, you can integrate your plugin with Dabra as follows:

1. Import it into [Governance UI](https://github.com/dabra-foundation/dabradao)
2. Register it in [VoterWeightPlugins/clients/index.ts](https://github.com/dabra-foundation/dabradao/blob/master/VoterWeightPlugins/clients/index.ts)
3. List it in [VotingStructureSelector/index.tsx](https://github.com/dabra-foundation/dabradao/blob/master/hub/components/EditDabraConfig/VotingStructureSelector/index.tsx) and [constants/plugins.ts](https://github.com/dabra-foundation/dabradao/blob/master/constants/plugins.ts) to ensure it appears in the list of plugins when editing a DAO.
...
To add your own:
- add your plugin name to `pluginsWithDedicatedVotingPowerUI` in [VotingPowerCards.tsx](https://github.com/dabra-foundation/dabradao/blob/master/components/GovernancePower/Power/VotingPowerCards.tsx)
- register your UI component inside `CardForPlugin` in the same file

## Voter Weight Plugin Chaining

Dabra supports chaining of voter weight plugins. This means that a voter's weight can be modified by multiple plugins
in order.

Plugins are structured like a linked-list, where each plugin registrar has a reference to the previous plugin program ID.
The final plugin program ID is registered against the governance on the dabra.

When a voter weight is calculated, the Dabra UI will call the `update_voter_weight_record` instruction on each plugin in order.
...
in the [Governance Program Library](https://github.com/dabra-foundation/governance-program-library).
...
The Dabra code will automatically wire up your plugin with others in the chain, as long as your plugin client
provides a Registrar account that contains a `previousVoterWeightPluginProgramId` property.

If `previousVoterWeightPluginProgramId` is found, then Dabra will pass in an input VWR belonging to that plugin
...
class MyPluginClient extends Client<typeof IDL> {
    //...other functions
    getRegistrarAccount(dabra: PublicKey, mint: PublicKey) {
    ...
    async calculateVoterWeight(voter: PublicKey, dabra: PublicKey, mint: PublicKey, inputVoterWeight: BN): Promise<BN | null> {
        const registrar = await this.getRegistrarAccount(dabra, mint);
